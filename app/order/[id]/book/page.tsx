...
export default function BookOrder({ params }: PageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Cari pekerja tersedia
      const { data: availableWorker, error: workerError } = await supabase
        .from('workers')
        .select('id')
      ...
      if (!availableWorker || availableWorker.length === 0) {
        setError('Maaf, belum ada pekerja yang tersedia saat ini')
        setLoading(false)
        return
      }
      ...
      // 3. Update status pekerja menjadi tidak tersedia
      await supabase
        .from('workers')
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8 text-center">
          ...
          <p className="text-gray-600 mb-4">
            Pekerja akan segera menghubungi kamu.
          </p>
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-lg mx-auto bg-white rounded-xl shadow-lg p-6">
        <div className="flex items-center gap-2 mb-6">
          <Link href={`/order/${params.id}`} className="text-gray-500 hover:text-gray-700">
            ←
          </Link>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">Kosongkan untuk datang sekarang</p>
          </div>
        ...
        <p className="text-center text-xs text-gray-500 mt-4">
          Dengan memesan, kamu setuju dengan syarat dan ketentuan Gocraft
        </p>
      </div>
    </div>
  )
